const racks = [
  ...
  let addMedicneArray = [];
  let i = 0;
  // Save the addMedicneArray data to local storage
function saveToLocalStorage() {
    localStorage.setItem('addMedicneArray', JSON.stringify(addMedicneArray));
  }
  
  // Call this function whenever you modify the addMedicneArray
  function updateLocalStorageAndTable() {
    saveToLocalStorage();
    updateMedicineTable();
  }
  ...
  // Load the addMedicneArray data from local storage
function loadFromLocalStorage() {
    const storedData = localStorage.getItem('addMedicneArray');
    if (storedData) {
      addMedicneArray = JSON.parse(storedData);
      updateMedicineTable();
    }
  }
  ...
  // Save the addRecodeArray data to local storage
function saveRecodeToLocalStorage() {
    localStorage.setItem('addRecodeArray', JSON.stringify(addRecodeArray));
  }
  
  // Call this function whenever you modify the addRecodeArray
  function updateRecodeLocalStorageAndTable() {
    saveRecodeToLocalStorage();
    displayRecordsInTable();
  }
// Load the addRecodeArray data from local storage
function loadRecodeFromLocalStorage() {
    const storedData = localStorage.getItem('addRecodeArray');
    if (storedData) {
      addRecodeArray = JSON.parse(storedData);
      displayRecordsInTable();
    }
  }
  
  // Call this function when your page loads
  loadRecodeFromLocalStorage();
  ...
  addRecodeForm.addEventListener("submit", (e) => {
    ...
    if (addRecodeSerchMedicine.value == "") {
      ...
      if (existingRecordIndex !== -1) {
        ...
        updateRecodeLocalStorageAndTable();
      } else {
  ...
  function removeRecodeRow(id) {
    addRecodeArray = addRecodeArray.filter((item) => item.id !== id);
    updateRecodeLocalStorageAndTable();
  }
  ...
  let p = 0;
  let outRecode;
  // Save the addOutRecordArray data to local storage
function saveOutRecordToLocalStorage() {
    localStorage.setItem('addOutRecordArray', JSON.stringify(newOutRecordArray));
  }
  
  // Call this function whenever you modify the newOutRecordArray
  function updateOutRecordLocalStorageAndTable() {
    saveOutRecordToLocalStorage();
    updateOutRecordTable();
  }
// Load the addOutRecordArray data from local storage
function loadOutRecordFromLocalStorage() {
    const storedData = localStorage.getItem('addOutRecordArray');
    if (storedData) {
      newOutRecordArray = JSON.parse(storedData);
      updateOutRecordTable();
    }
  }
  

  loadOutRecordFromLocalStorage();
...
function addOutRowToTable(record) {
    ...
    saveOutRecordToLocalStorage(); // Save to local storage after adding a new row
  }
  ...
  newOutForm.addEventListener("submit", (e) => {
    ...
    if (
      ...
      if (correspondingRecordIndex !== -1) {
        const remainingQuantity = addRecodeArray[correspondingRecordIndex].recodeQuantity - parseInt(outquntity.value);
        if (remainingQuantity < 0) {
          addRecodeArray[correspondingRecordIndex].recodeQuantity = 0;
        } else {
          addRecodeArray[correspondingRecordIndex].recodeQuantity = remainingQuantity;
        }
      }
      ...
      displayRecordsInTable();
      // updateMedicineTable();
      updateOutRecordLocalStorageAndTable();
    }
  });
...
function updateOutRecordTable() {
  ...
  saveRecodeToLocalStorage(); // Save to local storage after updating the out record table
}
  ...
  function updateRow(id) {
    const itemToUpdate = addMedicneArray.find((item) => item.id === id);
    if (itemToUpdate) {
      ...
      addMedicineForm.style.display = "block";
      editingItemId = null;
      updateMedicineTable(); // Update table with the changes
      saveMedicineToLocalStorage(); // Update local storage after editing
    }
  }
  
  UpdateTable.addEventListener("click", () => {
    updateRow(editingItemId); // Call the updateRow function
  });
  // Save the addMedicneArray data to local storage
function saveMedicineToLocalStorage() {
    localStorage.setItem('addMedicneArray', JSON.stringify(addMedicneArray));
